...
class BestAgentAttackerNoNebulaDifferentExploreNoAttack:
    ...
    def send_to_explore_if_not_going_to_relic(
        ...
        """
        This method will assign units to unseen squares of the map (where seen_map[x,y] == False).
        We create a cost matrix where cost ~ distance from a unit to each candidate target. Then
        we use Hungarian assignment to minimize total cost. That way, we "spread out" over unseen squares.
        """

        if len(remaining_units) == 0:
            return  # Nothing to do

        # 1) Collect all unseen tiles
        unseen_coords = np.column_stack(np.where(self.seen_map == False))
        # unseen_coords will be an array of shape (N, 2), each row is [x, y].
        # If there's no unseen tile left, just idle everyone.
        if unseen_coords.shape[0] == 0:
            for u in remaining_units:
                actions[u] = [0, 0, 0]
            return

        # 2) If there are more unseen tiles than units, pick a subset.
        #    For example, randomly sample 'len(remaining_units)' many unseen tiles (or some multiple).
        num_units = len(remaining_units)
        if unseen_coords.shape[0] > num_units:
            # Randomly sample a subset of unseen coords that is at least the number of units
            # or whichever logic you want
            idxs = np.random.choice(unseen_coords.shape[0], size=num_units, replace=False)
            target_coords = unseen_coords[idxs]
        else:
            # If there aren't many unseen tiles, just use them all
            target_coords = unseen_coords

        used_cell_count = len(target_coords)  # how many potential squares
        if used_cell_count == 0:
            # no unseen squares remain
            for u in remaining_units:
                actions[u] = [0, 0, 0]
            return

        # 3) Build a cost matrix for Hungarian assignment
        ...
        for i, unit_id in enumerate(remaining_units):
            ...
            for j, (tx, ty) in enumerate(target_coords):
                dist = abs(ux - tx) + abs(uy - ty)
                cost = dist
                # Optionally add penalty/bonus for known reward or not_reward (left as an example):
                if (tx, ty) in self.known_reward_tiles:
                    cost += REWARD_BONUS
                    if cost < 0:
                        cost = 0
        ...
        # 4) Perform Hungarian assignment
        row_ind, col_ind = linear_sum_assignment(cost_matrix)

        # 5) Assign moves toward the chosen targets
        assigned_units = set()
        unit_to_target = {}
        for r, c in zip(row_ind, col_ind):
            # only assign up to 'used_cell_count' units (in case #units > #targets)
            if r < num_units and c < used_cell_count:
                unit_id = remaining_units[r]
                tx, ty = target_coords[c]
                unit_to_target[unit_id] = (tx, ty)
