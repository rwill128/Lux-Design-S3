...
class BestAgent2Explore5x5:
    def __init__(self, player: str, env_cfg) -> None:
        ...
        self.relic_area_visited = {}  # maps (rx, ry) -> set of visited (x, y)
        
        # Cost bonus for unvisited tiles in relic area
        self.UNVISITED_RELIC_TILE_BONUS = -50  # Same magnitude as REWARD_BONUS for equal priority
    ...
    def act(self, step: int, obs, remainingOverageTime: int = 60):
        ...
        # 4) Track visited tiles in relic areas
        for uid in np.where(obs["units_mask"][self.team_id])[0]:
            x, y = obs["units"]["position"][self.team_id][uid]
            for (rx, ry) in self.relic_area_visited.keys():
                if abs(x - rx) <= 2 and abs(y - ry) <= 2:  # Within 5x5 area of relic
                    self.relic_area_visited[(rx, ry)].add((x, y))
            ...
            for i, u in enumerate(remaining_units):
                ...
                for j, (tx, ty) in enumerate(relic_targets):
                    ...
                    if (tx, ty) in self.known_reward_tiles:
                        cost += REWARD_BONUS
                    elif (tx, ty) in self.not_reward_tiles:
                        cost += NON_REWARD_PENALTY
                    else:
                        # Check if tile is unvisited in any relic's 5x5 area
                        for (rx, ry), visited in self.relic_area_visited.items():
                            if abs(tx - rx) <= 2 and abs(ty - ry) <= 2:  # Within 5x5 area
                                if (tx, ty) not in visited:  # Unvisited tile
    ...
    def add_newly_discovered_relics(self, relic_nodes_positions):
        ...
        for (rx, ry) in relic_nodes_positions:
            # Initialize relic tile data if not already present
            ...
            # Initialize visited tiles tracking if not already present
            if (rx, ry) not in self.relic_area_visited:
...
def agent_fn(observation, configurations):
    ...
    if step == 0:
        agent_dict[player] = BestAgent2Explore5x5(player, configurations["env_cfg"])
