...
class BestAgentAttackerFive:
    """
    Fifth iteration of the BestAgentAttacker with enhanced capabilities:
    - Improved reward tile detection with confidence tracking
    - Enhanced pathfinding with energy and nebula awareness
    - Optimized unit coordination and target selection
    - Better energy management and combat tactics
    """
    ...
    def get_direction_via_pathfinding(self, from_pos, to_pos, obs):
        ...
        assert isinstance(action, int), (
            f"Action is not an integer! Found: {action} (type: {type(action)})"
        )
        return action
    ...
    def select_tiles_for_relic(self, relic_pos):
        # ... unchanged ...
        reward_tiles = [
            t for t, d in self.relic_tile_data[relic_pos].items()
            if d["reward_tile"]
        ]
        untested_tiles = [
            t for t, d in self.relic_tile_data[relic_pos].items()
    ...
    def deduce_reward_tiles(self, obs):
        ...
        # Update visit counts and confidence scores based on point gains and relic proximity
        relic_nodes_mask = obs["relic_nodes_mask"]
        relic_nodes = obs["relic_nodes"][relic_nodes_mask]
        relic_positions = set((int(x), int(y)) for x, y in relic_nodes)
        
        # First pass: Update visit counts and handle confirmed tiles
        for pos in occupied_this_turn:
            self.tile_visit_count[pos] = self.tile_visit_count.get(pos, 0) + 1
            
            if pos in self.known_reward_tiles:
                self.tile_confidence[pos] = 100  # Maximum confidence for confirmed reward tiles
            elif pos in self.not_reward_tiles:
                self.tile_confidence[pos] = 0    # Zero confidence for confirmed non-reward tiles
        
        # Second pass: Update confidence for unknown tiles
        for pos in self.unknown_tiles:
            x, y = pos
            current_confidence = self.tile_confidence.get(pos, 50)  # Default 50% confidence
            
            # Factor 1: Point gains for newly occupied tiles
            if pos in newly_occupied:
                if gain > 0 and len(newly_occupied) == 1:
                    current_confidence = 100  # Confirmed reward tile
                elif gain == 0:
                    visits = self.tile_visit_count.get(pos, 0)
                    decay = min(30, 10 * visits)  # Stronger decay with visits
                    current_confidence = max(0, current_confidence - decay)
            
            # Factor 2: Proximity to relics
            min_dist = float('inf')
            for rx, ry in relic_positions:
                dist = abs(x - rx) + abs(y - ry)  # Manhattan distance
                min_dist = min(min_dist, dist)
            
            # Boost confidence for unvisited tiles near relics
            if min_dist <= 2 and self.tile_visit_count.get(pos, 0) == 0:
                proximity_bonus = max(0, 20 - (min_dist * 5))  # 20% bonus at dist=0, decreasing by 5% per tile
                current_confidence = min(100, current_confidence + proximity_bonus)
            
            # Factor 3: Visit count penalties
            visits = self.tile_visit_count.get(pos, 0)
            if visits > 0:
                visit_penalty = min(50, visits * 10)  # Up to 50% penalty based on visits
                current_confidence = max(0, current_confidence - visit_penalty)
            
            self.tile_confidence[pos] = current_confidence
        ...
        def is_passable(x, y):
            ...
            if sensor_mask[x, y]:
                ...
                if obs["board"]["nebulas"][y, x]:
                    ...
                    relic_nodes = obs["relic_nodes"][relic_nodes_mask]
                    for rx, ry in relic_nodes:
                        if abs(x - int(rx)) <= 2 and abs(y - int(ry)) <= 2:
                            return True  # Allow nebula near relic
                    return False  # Block nebula away from relics
                return True
            else:
        ...
        while pq:
            ...
            for dx, dy in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
                ...
                if 0 <= nx < map_width and 0 <= ny < map_height and is_passable(nx, ny):
                    # Calculate base cost from energy
                    ...
                    pos = (nx, ny)
                    confidence = self.tile_confidence.get(pos, 50)
                    confidence_penalty = int((100 - confidence) * 0.2)  # Lower confidence = higher cost
                    tile_cost += confidence_penalty
                    
                    # Add visit count penalty
                    visits = self.tile_visit_count.get(pos, 0)
                    visit_penalty = min(20, visits * 5)  # Up to +20 cost based on visits
                    ...
                    if obs["board"]["nebulas"][ny, nx]:
                        ...
                        near_relic = False
                        for rx, ry in relic_nodes:
                            if abs(nx - int(rx)) <= 2 and abs(ny - int(ry)) <= 2:
                                near_relic = True
                                break
                        if not near_relic:
                            tile_cost += 50  # Heavy penalty for nebulas away from relics
    ...
    def send_to_attack_if_not_going_to_relic(
        self,
        NON_REWARD_PENALTY,
        REWARD_BONUS,
        actions,
    ...
    def send_to_explore_if_not_going_to_relic(
        self,
        NON_REWARD_PENALTY,
        REWARD_BONUS,
        actions,
    ...
    def send_to_relic_points(self, actions, map_height, map_width, obs, remaining_units, unit_positions):
        # Handle units already on reward tiles
    ...
    def send_toward_relic_areas(
            self, actions, available_unit_ids, map_height, map_width,
            obs, sap_done, unit_positions
    ):
        remaining_units = [u for u in available_unit_ids if u not in sap_done]
        ...
        relic_targets = list(set(
            list(set(relic_targets) - set(self.not_reward_tiles)) +
            list(self.known_reward_tiles)
        ))
    ...
    def do_sapping_logic(
            self, actions, available_unit_ids, enemy_positions,
            sap_cost, sap_done, sap_range, unit_energy,
            unit_positions, opp_energy
    ):
...
def agent_fn(observation, configurations):
    ...
    if step == 0:
        agent_dict[player] = BestAgentAttackerFive(player, configurations["env_cfg"])
