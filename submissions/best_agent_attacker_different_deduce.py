...
class BestAgentAttackerDifferentDeduce:
    ...
    def deduce_reward_tiles(self, obs):
        ...
        if gain_rate == 0:
            ...
            if len(newly_occupied) > 1 and len(newly_unoccupied) > 1:
                self.known_reward_tiles -= newly_occupied
                self.known_reward_tiles -= newly_unoccupied
                self.unknown_tiles.update(newly_occupied)
                self.unknown_tiles.update(newly_unoccupied)
...
def agent_fn(observation, configurations):
    ...
    if step == 0:
        agent_dict[player] = BestAgentAttackerDifferentDeduce(player, configurations["env_cfg"])
